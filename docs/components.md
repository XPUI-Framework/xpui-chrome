...
passes in. No
backend takes that route yet: `xpui-embedded-graphics` takes all nine through
the macro, and the FreeInkUI backend answers `Chrome` over its own C ABI
without depending on this crate. Why it is a macro at all is in
[design.md](design.md).
