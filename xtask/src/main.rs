//! The gate for `xpui-chrome`.
//!
//! Everything CI checks, in one command, and **only what this repository has
//! to check**. There is no C++ here, so there is no C++ stage; there is no
//! product name to forbid, so there is no genericity grep. A gate carrying
//! checks its repository never runs is a gate nobody reads, and this one used
...
/// Clippy, on the host and on each bare-metal target, with warnings as errors.
///
/// The host build never parses code behind `cfg(target_os = "none")`, so the
/// bare-metal runs are the only gates that reach it before a firmware build
/// does. Neither target has atomic compare-and-swap — load and store only.
            ...
            if required {
                return Err(format!(
                    "{triple} is not installed, and it is the only gate that reaches\n\
                     this crate's no_std paths. `rustup target add {triple}`"
                ));
            }
